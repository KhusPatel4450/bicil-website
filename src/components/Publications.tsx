...
export default function Publications({ compact = false }: { compact?: boolean }) {
  ...
  const displayed = compact ? PUBS.slice(0, 3) : filtered;
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-4">
            {compact ? "Recent Work" : "All Publications"}
          </h2>
          <p className="text-slate-600 max-w-xl leading-relaxed">
            {compact
              ? "A selection of published research from top-tier journals and internationally peer-reviewed conferences."
              : "Peer-reviewed publications from top-tier journals and internationally recognised conferences."}
          </p>
        </motion.div>
        ...
        <div className="divide-y divide-slate-200" role="list" aria-label="Publication list">
          {compact ? (
            displayed.map((pub, i) => <PubCard key={pub.title} pub={pub} i={i} />)
          ) : (
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mt-10 pt-8 border-t border-slate-200 flex flex-col sm:flex-row items-start sm:items-center gap-4"
        >
          {compact && (
            <Link
              href="/research#publications"
              className="inline-flex items-center gap-2 px-6 py-2.5 border border-slate-300 hover:border-[#2A7FC1] text-slate-600 hover:text-[#2A7FC1] font-medium text-sm transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#4BBFCF]"
            >
